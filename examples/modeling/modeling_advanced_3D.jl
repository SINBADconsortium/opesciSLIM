...
# receiver x position
xrec = Array{Any}(nsrc)
xrec[1] = linspace(150., 1150., nxrec[1])	# 120 receivers in x direction for experiment 1
xrec[2] = linspace(180., 1000., nxrec[2])		# 60 receivers in x direction for experiment 2
xrec[3] = linspace(250., 1050., nxrec[3])	# 30 receivers in x direction for experiment 3
...
opt = Options(limit_m=true,		# limit modeling domain to area with receivers (does not change data, gradients)
			  buffer_size=500	# when limiting modeling domain, keep buffer area on each side, default is 500 m
			  )
