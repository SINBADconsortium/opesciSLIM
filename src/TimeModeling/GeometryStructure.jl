...
(2) Set up corresponding source geometry (coordinates can be of type `linspace` or regular arrays):
    ...
    xsrc = convertToCell([200,400,600,800])
    ysrc = convertToCell([0,0,0,0])
    zsrc = convertToCell([50,50,50,50])

Coordinates are already cell arrays, so no `nsrc` keyword necessary:
