...
function fwi_objective(model_full::Model, source::joData, dObs::joData, srcnum::Int64; options=Options())
# Setup time-domain linear or nonlinear foward and adjoint modeling and interface to OPESCI/devito 
	...
	if length(model.n) == 2
		...
		dimsflip = (2,1)
		dimsfull = nz+model.nb*2, nx+model.nb*2	# dims for devito
	else
		nx, ny, nz = model.n
		damp = zeros(Float32,nx, ny, nz)
		dimsflip = (3,2,1)
		dimsfull = nz+model.nb*2, ny+model.nb*2, nx+model.nb*2	# dims for devito
	end

	# Set up model structure
	modelPy = ct.IGrid(model.o, model.d, model.n, sqrt.(1./model.m),nbpml=model.nb)
	...
	types, args = process_args(op, Dict(:damp=>damp, :m=>m, :rec=>dPredicted, :rec_coords=>rec_coords', :src=>qIn, :src_coords=>src_coords', :u=>u, :usave=>up))
	lib = op[:compile]
	...
	types, args = process_args(opg, Dict(:damp=>damp, :m=>m, :rec=>residual, :rec_coords=>rec_coords', :u=>up, :grad=>gradient, :v=>v))
	lib = opg[:compile]
